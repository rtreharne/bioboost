...
const PRACTICE_FOCUS_REMINDER = (
  "A quick reminder: staying with Test Mode from start to finish gives the clearest picture of what you know securely and what still needs work. If you can, try to avoid switching away while you're in the test."
);
...
const validationPreviewDataNode = document.getElementById("validation-preview-data");
...
if (validationRoot && validationDataNode) {
  ...
  const demoEmbedOriginTokenUrl = String(validationRoot.dataset.demoEmbedOriginTokenUrl || "").trim();
  ...
  const validationMeta = validationRoot.querySelector("[data-validation-meta]");
  ...
  const courseMetricsPanel = validationRoot.querySelector("[data-preview-course-metrics]");
  const blockSwitcher = validationRoot.querySelector(".preview-block-switcher");
  ...
  const returnToPracticeButton = validationRoot.querySelector("[data-return-to-practice]");
  ...
  const practiceUrl = String(validationRoot.dataset.practiceUrl || "").trim();
  const practicePreviewMode = String(validationRoot.dataset.practicePreviewMode || "").trim();
  const courseId = String(validationRoot.dataset.courseId || "").trim();
  const leaveValidationWarning = String(validationRoot.dataset.leaveValidationWarning || "").trim();
  const restartValidationUrl = String(validationRoot.dataset.restartValidationUrl || "").trim();
  ...
  const sidebarPreviewState = validationPreviewDataNode ? JSON.parse(validationPreviewDataNode.textContent || "{}") : {};
  ...
  let offlinePauseStatusVisible = false;
  let practiceFocusReminderPending = false;
  let practiceValidationNavigationTimer = 0;
  let testModeSetupStage = "blocks";
  let testModeBaseSelectAll = true;
  let testModeSelectedCollectionIds = new Set();
  let testModePinnedBlockIds = new Set();
  let testModeSuppressedBlockIds = new Set();
  let demoEmbedOriginTokenPromise = null;
  let sidebarActiveBlockId = String(sidebarPreviewState.active_block_id || "");
  let testModeSelectedBlockIds = new Set();
  let testModeQuestionCount = 0;
  ...
  function isPracticeValidationMode() {
    return String(sessionState.mode || "") === "validation_practice";
  }

  function isValidationSessionMode(mode = String(sessionState.mode || "")) {
    return ["validation_practice", "digital_invigilation", "preview_validate"].includes(String(mode || ""));
  }
  ...
  function showPracticeFocusReminder() {
    practiceFocusReminderPending = false;
    setStatus(PRACTICE_FOCUS_REMINDER);
  }
  ...
  function syncOfflineValidationStatus() {
    if (!isValidationSessionMode() || sessionState.completed) {
      if (offlinePauseStatusVisible) {
        offlinePauseStatusVisible = false;
        setStatus("");
      }
      return;
    }
    if (isBrowserOffline()) {
      offlinePauseStatusVisible = true;
      setStatus("Connection lost. Anti-navigation cancellation is paused while you are offline. Reconnect before submitting.");
      return;
    }
    if (offlinePauseStatusVisible) {
      offlinePauseStatusVisible = false;
  ...
  function normalizeCancelledSessionState() {
    const invalidatedReason = String(sessionState.invalidated_reason || "").trim();
    ...
    sessionState = {
      ...
      transcript: [
        {
          id: "validation-cancelled-message",
          role: "assistant",
          kind: "summary",
          text: invalidatedReason,
        },
        ...(restartValidationUrl
          ? [{
            id: "validation-cancelled-restart",
            role: "assistant",
            kind: "cta",
            text: "",
            actions: [{ label: "Restart Validation", url: restartValidationUrl, style: "button" }],
          }]
          : []),
      ],
    };
  ...
  function metricLabel(metricKey, scope = "block") {
    switch (String(metricKey || "")) {
      case "overall":
        return scope === "course" ? "Overall course score" : "Overall score";
      case "mastery":
        return "Mastery";
      case "coverage":
        return "Coverage";
  ...
  function practiceBlockStorageKey() {
    return courseId && practicePreviewMode
      ? `quizanchor:${practicePreviewMode}:course:${courseId}:active-block`
      : "";
  }

  function persistPracticeBlockId(blockId) {
    const storageKey = practiceBlockStorageKey();
    if (!storageKey) {
      return;
    }
    try {
  ...
  function practiceReturnUrl(blockId = "") {
    if (!practiceUrl) {
      return "";
    }
    try {
      ...
      if (blockId) {
        nextUrl.searchParams.set("block", String(blockId));
      } else {
        nextUrl.searchParams.delete("block");
      }
      return nextUrl.toString();
    } catch (_error) {
      return practiceUrl;
  ...
  function navigateToPracticeBlock(blockId = "") {
    if (!isValidationSessionMode()) {
      return;
    }
    void cancelValidationSession();
  }

  function selectableBlocks() {
    return Array.isArray(sessionState.selectable_blocks) ? sessionState.selectable_blocks : [];
  }

  function selectableCollections() {
    return Array.isArray(sessionState.selectable_collections) ? sessionState.selectable_collections : [];
  }

  function maxTestModeQuestionCount() {
    const configuredMax = Number.parseInt(String(sessionState.max_question_count || ""), 10);
    if (Number.isFinite(configuredMax) && configuredMax > 0) {
      return Math.min(configuredMax, 20);
    }
    return 20;
  }

  function defaultTestModeQuestionCountFromSession() {
    const configuredMax = maxTestModeQuestionCount();
    const sessionCount = Number.parseInt(String(sessionState.selected_question_count || ""), 10);
    if (Number.isFinite(sessionCount) && sessionCount >= 1) {
      return Math.min(sessionCount, configuredMax);
    }
    const progressCount = Number.parseInt(String(sessionState.progress?.total_questions || ""), 10);
    if (Number.isFinite(progressCount) && progressCount >= 1) {
      return Math.min(progressCount, configuredMax);
    }
    return Math.min(10, configuredMax);
  }

  function defaultSelectedBlockIdsFromSession() {
    const availableBlockIds = selectableBlocks()
      .map((block) => String(block?.id || "").trim())
      .filter(Boolean);
    const allowedBlockIds = new Set(availableBlockIds);
    const sessionSelectedIds = Array.isArray(sessionState.selected_block_ids)
      ? sessionState.selected_block_ids
        .map((blockId) => String(blockId || "").trim())
        .filter((blockId) => blockId && allowedBlockIds.has(blockId))
      : [];
    return sessionSelectedIds.length ? sessionSelectedIds : availableBlockIds;
  }

  function selectableBlockIds() {
    return selectableBlocks()
      .map((block) => String(block?.id || "").trim())
      .filter(Boolean);
  }

  function collectionBlockIds(collection) {
    return Array.isArray(collection?.block_ids)
      ? collection.block_ids.map((blockId) => String(blockId || "").trim()).filter(Boolean)
      : [];
  }

  function impliedCollectionBlockIds(activeCollectionIds = testModeSelectedCollectionIds) {
    const selectedIds = new Set(Array.from(activeCollectionIds).map((collectionId) => String(collectionId)));
    const blockIds = new Set();
    selectableCollections().forEach((collection) => {
      const collectionId = String(collection?.id || "").trim();
      if (!collectionId || !selectedIds.has(collectionId)) {
        return;
      }
      collectionBlockIds(collection).forEach((blockId) => {
  ...
  function recomputeTestModeSelectedBlockIds() {
    const allowedBlockIds = new Set(selectableBlockIds());
    const nextSelectedBlockIds = new Set();
    if (testModeBaseSelectAll && !testModeSelectedCollectionIds.size) {
      allowedBlockIds.forEach((blockId) => {
        nextSelectedBlockIds.add(blockId);
      });
    } else {
      impliedCollectionBlockIds().forEach((blockId) => {
        if (allowedBlockIds.has(blockId)) {
          nextSelectedBlockIds.add(blockId);
        }
      });
      testModePinnedBlockIds.forEach((blockId) => {
        if (allowedBlockIds.has(blockId)) {
          nextSelectedBlockIds.add(blockId);
        }
      });
    }
    testModeSuppressedBlockIds.forEach((blockId) => {
      nextSelectedBlockIds.delete(blockId);
    });
    if (!nextSelectedBlockIds.size) {
      const fallbackBlockId = selectableBlockIds()[0];
      if (fallbackBlockId) {
        nextSelectedBlockIds.add(fallbackBlockId);
        testModePinnedBlockIds.add(fallbackBlockId);
        testModeSuppressedBlockIds.delete(fallbackBlockId);
      }
    }
    testModeSelectedBlockIds = nextSelectedBlockIds;
  }

  function resetTestModeSelectionState() {
    const defaultSelectedIds = defaultSelectedBlockIdsFromSession();
    const allBlockIds = selectableBlockIds();
    testModeSelectedCollectionIds = new Set();
    testModePinnedBlockIds = new Set();
    testModeSuppressedBlockIds = new Set();
    testModeBaseSelectAll = (
      defaultSelectedIds.length === allBlockIds.length
      && defaultSelectedIds.every((blockId) => allBlockIds.includes(blockId))
    );
    if (!testModeBaseSelectAll) {
      defaultSelectedIds.forEach((blockId) => {
        testModePinnedBlockIds.add(blockId);
      });
    }
    recomputeTestModeSelectedBlockIds();
  }

  function syncSelectedBlockIdsFromSession({ force = false } = {}) {
    const allowedBlockIds = new Set(
      selectableBlockIds(),
    );
    if (force || !testModeSelectedBlockIds.size) {
      resetTestModeSelectionState();
      return;
    }
    testModeSelectedCollectionIds = new Set(
      Array.from(testModeSelectedCollectionIds).filter((collectionId) => {
        const normalizedCollectionId = String(collectionId || "").trim();
        return normalizedCollectionId && selectableCollections().some(
          (collection) => String(collection?.id || "").trim() === normalizedCollectionId,
        );
      }),
    );
    testModePinnedBlockIds = new Set(
      Array.from(testModePinnedBlockIds).filter((blockId) => allowedBlockIds.has(String(blockId))),
    );
    testModeSuppressedBlockIds = new Set(
      Array.from(testModeSuppressedBlockIds).filter((blockId) => allowedBlockIds.has(String(blockId))),
    );
    testModeSelectedBlockIds = new Set(
      Array.from(testModeSelectedBlockIds).filter((blockId) => allowedBlockIds.has(String(blockId))),
    );
    recomputeTestModeSelectedBlockIds();
  }

  function syncTestModeQuestionCountFromSession({ force = false } = {}) {
    const defaultCount = defaultTestModeQuestionCountFromSession();
    if (force || !testModeQuestionCount) {
      testModeQuestionCount = defaultCount;
      return;
    }
    const configuredMax = maxTestModeQuestionCount();
    testModeQuestionCount = Math.max(1, Math.min(configuredMax, Number.parseInt(String(testModeQuestionCount || defaultCount), 10) || defaultCount));
  }

  function isTestModeSetupActive() {
    return (
      String(sessionState.mode || "") === "validation_practice"
      && !sessionState.completed
      && !sessionState.instructions_confirmed
      && selectableBlocks().length > 0
    );
  }

  function isTestModeCountSetupStage() {
    return isTestModeSetupActive() && testModeSetupStage === "count";
  }

  function syncTestModeSetupStage() {
    if (!isTestModeSetupActive()) {
      testModeSetupStage = "blocks";
      return;
    }
    if (!["blocks", "count"].includes(String(testModeSetupStage || ""))) {
      testModeSetupStage = "blocks";
    }
  }

  function selectedSidebarBlocks() {
    const allBlocks = Array.isArray(sidebarPreviewState.blocks) ? sidebarPreviewState.blocks : [];
    const selectedIds = new Set(defaultSelectedBlockIdsFromSession());
    if (isTestModeSetupActive()) {
      selectedIds.clear();
      Array.from(testModeSelectedBlockIds).forEach((blockId) => {
        selectedIds.add(String(blockId));
      });
    }
    if (!selectedIds.size) {
      return allBlocks;
    }
    const filteredBlocks = allBlocks.filter((block) => selectedIds.has(String(block.id || "")));
    return filteredBlocks.length ? filteredBlocks : allBlocks;
  }

  function collectionIsSelected(collectionId) {
    const normalizedCollectionId = String(collectionId || "").trim();
    return normalizedCollectionId ? testModeSelectedCollectionIds.has(normalizedCollectionId) : false;
  }

  function applyTestModeCollection(collection) {
    const collectionId = String(collection?.id || "").trim();
    const allowedBlockIds = new Set(selectableBlockIds());
    const normalizedIds = collectionBlockIds(collection).filter((blockId) => allowedBlockIds.has(blockId));
    if (!normalizedIds.length) {
      return;
    }
    if (collectionId && testModeSelectedCollectionIds.has(collectionId)) {
      const nextCollectionIds = new Set(testModeSelectedCollectionIds);
      nextCollectionIds.delete(collectionId);
      const remainingImpliedBlockIds = impliedCollectionBlockIds(nextCollectionIds);
      const remainingSelectedBlockIds = new Set();
      if (testModeBaseSelectAll && !nextCollectionIds.size) {
        allowedBlockIds.forEach((blockId) => {
          if (!testModeSuppressedBlockIds.has(blockId)) {
            remainingSelectedBlockIds.add(blockId);
          }
        });
      } else {
        remainingImpliedBlockIds.forEach((blockId) => {
          if (!testModeSuppressedBlockIds.has(blockId)) {
            remainingSelectedBlockIds.add(blockId);
          }
        });
        testModePinnedBlockIds.forEach((blockId) => {
          if (allowedBlockIds.has(blockId) && !testModeSuppressedBlockIds.has(blockId)) {
            remainingSelectedBlockIds.add(blockId);
          }
        });
      }
      if (!remainingSelectedBlockIds.size) {
        return;
      }
      testModeSelectedCollectionIds = nextCollectionIds;
      recomputeTestModeSelectedBlockIds();
      renderTranscript({ preserveScroll: true });
      return;
    }
    const allBlocksCurrentlySelected = Array.from(allowedBlockIds).every((blockId) => testModeSelectedBlockIds.has(blockId));
    if (allBlocksCurrentlySelected && !testModeSelectedCollectionIds.size && !testModePinnedBlockIds.size && !testModeSuppressedBlockIds.size) {
      testModeBaseSelectAll = false;
      testModeSelectedCollectionIds = new Set(collectionId ? [collectionId] : []);
      testModePinnedBlockIds = new Set();
      testModeSuppressedBlockIds = new Set();
      recomputeTestModeSelectedBlockIds();
      renderTranscript({ preserveScroll: true });
    ...
    if (collectionId) {
      testModeSelectedCollectionIds.add(collectionId);
    }
    normalizedIds.forEach((blockId) => {
      testModeSuppressedBlockIds.delete(blockId);
    });
    recomputeTestModeSelectedBlockIds();
    renderTranscript({ preserveScroll: true });
  }

  function toggleAllTestModeBlocks() {
    const blocks = selectableBlocks();
    if (!blocks.length) {
      return;
    }
    const blockIds = blocks.map((block) => String(block.id));
    const allSelected = blockIds.every((blockId) => testModeSelectedBlockIds.has(blockId));
    testModeSelectedCollectionIds = new Set();
    testModePinnedBlockIds = new Set();
    testModeSuppressedBlockIds = new Set();
    if (allSelected) {
      testModeBaseSelectAll = false;
      testModePinnedBlockIds.add(blockIds[0]);
    } else {
      testModeBaseSelectAll = true;
    }
    recomputeTestModeSelectedBlockIds();
    renderTranscript({ preserveScroll: true });
  }

  function toggleTestModeBlock(blockId) {
    const normalizedBlockId = String(blockId || "").trim();
    if (!normalizedBlockId) {
      return;
    }
    const impliedBlockIds = impliedCollectionBlockIds();
    if (testModeSelectedBlockIds.has(normalizedBlockId)) {
      if (testModeSelectedBlockIds.size <= 1) {
        return;
      }
      if (testModeBaseSelectAll && !testModeSelectedCollectionIds.size) {
        testModeSuppressedBlockIds.add(normalizedBlockId);
      } else if (impliedBlockIds.has(normalizedBlockId)) {
        testModeSuppressedBlockIds.add(normalizedBlockId);
        testModePinnedBlockIds.delete(normalizedBlockId);
      } else {
        testModePinnedBlockIds.delete(normalizedBlockId);
      }
    } else {
      if (testModeBaseSelectAll && !testModeSelectedCollectionIds.size) {
        testModeSuppressedBlockIds.delete(normalizedBlockId);
      } else if (impliedBlockIds.has(normalizedBlockId)) {
        testModeSuppressedBlockIds.delete(normalizedBlockId);
      } else {
        testModePinnedBlockIds.add(normalizedBlockId);
      }
    }
    recomputeTestModeSelectedBlockIds();
    renderTranscript({ preserveScroll: true });
  ...
  function setTestModeQuestionCount(nextCount) {
    const configuredMax = maxTestModeQuestionCount();
    const normalizedCount = Math.max(1, Math.min(configuredMax, Number.parseInt(String(nextCount || ""), 10) || defaultTestModeQuestionCountFromSession()));
    testModeQuestionCount = normalizedCount;
    renderTranscript({ preserveScroll: true });
  }
  ...
  function renderCourseMetrics() {
    ...
    const metrics = sidebarPreviewState.course?.metrics;
    if (!metrics) {
    ...
    `;
    courseMetricsPanel.querySelectorAll("[data-preview-metric-button='true']").forEach((button) => {
      button.addEventListener("click", () => {
        navigateToPracticeBlock(sidebarActiveBlockId || String(sidebarPreviewState.blocks?.[0]?.id || ""));
      });
    });
  }
  ...
  function renderBlockSwitcher() {
    ...
    syncSelectedBlockIdsFromSession({
      force: !!sessionState.completed || !!sessionState.instructions_confirmed,
    });
    syncTestModeQuestionCountFromSession({
      force: !!sessionState.completed || !!sessionState.instructions_confirmed,
    });
    blockSwitcher.innerHTML = "";
    const visibleBlocks = selectedSidebarBlocks();
    const activeVisibleBlockId = visibleBlocks.some((block) => String(block.id) === String(sidebarActiveBlockId))
      ? String(sidebarActiveBlockId)
      : String(visibleBlocks[0]?.id || "");
    sidebarActiveBlockId = activeVisibleBlockId;
    visibleBlocks.forEach((block) => {
      const isActive = String(block.id) === activeVisibleBlockId;
      const article = document.createElement("article");
      article.className = `preview-block-card${isActive ? " is-expanded is-active" : ""}`;

      const controlsId = `validation-sidebar-block-panel-${block.id}`;
      ...
      button.addEventListener("click", () => {
        navigateToPracticeBlock(block.id);
      });
      ...
      `;
      content.querySelectorAll("[data-preview-metric-button='true']").forEach((metricButton) => {
  ...
  function syncQuestionViewport({ preserveScroll = false } = {}) {
    if (!transcriptNode) {
      return;
    }
    ...
    if (preserveScroll || isTestModeSetupActive()) {
      return;
    }
  ...
  function syncControls() {
    ...
    const showTestModeSetupCta = isTestModeSetupActive();
    ...
    if (nextRow) {
      nextRow.hidden = !(showTestModeSetupCta || shouldShowNextButton());
    }
    if (submitButton) {
    ...
    if (nextButton) {
      if (showTestModeSetupCta) {
        nextButton.disabled = requestInFlight || (!isTestModeCountSetupStage() && testModeSelectedBlockIds.size <= 0);
        nextButton.textContent = requestInFlight && activeAction === "confirm"
          ? "Starting..."
          : (isTestModeCountSetupStage() ? "Start" : "Next");
      } else {
    ...
    const transcript = (
      transcriptMessages
        ? transcriptMessages
          ...
          .flatMap((message) => {
            if (message.kind !== "test_setup") {
              return [{ ...message }];
            }
            const setupMessages = [
              {
                ...message,
                id: `${String(message.id || "test-setup")}-blocks`,
                kind: "test_setup_blocks",
                text: "Choose the collections or course blocks to include in this Test Mode run.",
              },
            ];
            if (isTestModeCountSetupStage()) {
              setupMessages.push({
                ...message,
                id: `${String(message.id || "test-setup")}-count`,
                kind: "test_setup_count",
                text: "Choose how many questions to generate for this Test Mode run.",
              });
            }
  ...
  function submitTestModeSetup() {
    if (requestInFlight) {
      return;
    }
    const selectedBlockIds = Array.from(testModeSelectedBlockIds)
      .map((blockId) => Number.parseInt(String(blockId), 10))
      .filter((blockId) => Number.isFinite(blockId) && blockId > 0);
    if (!selectedBlockIds.length) {
      setStatus("Choose at least one course block for Test Mode.");
      return;
    }
    showLaunchLoader();
    void postAction("confirm", {
      selected_block_ids: selectedBlockIds,
      question_count: testModeQuestionCount || defaultTestModeQuestionCountFromSession(),
    });
  }
  ...
  function renderMessage(message) {
    ...
    if (message.kind === "test_setup_blocks") {
      appendFormattedMessageContent(article, message.text || "");
      const blocks = selectableBlocks();
      const collections = selectableCollections();
      const selectedCount = Array.from(testModeSelectedBlockIds).length;
      const selectionPanel = document.createElement("div");
      selectionPanel.className = "preview-test-setup";

      if (collections.length) {
        const collectionLabel = document.createElement("p");
        collectionLabel.className = "preview-written-answer-heading";
        collectionLabel.textContent = "Saved collections";
        selectionPanel.appendChild(collectionLabel);

        const collectionOptions = document.createElement("div");
        collectionOptions.className = "preview-message-options";
        collections.forEach((collection) => {
          const blockIds = Array.isArray(collection.block_ids) ? collection.block_ids : [];
          const isSelected = collectionIsSelected(collection.id);
          const collectionButton = document.createElement("button");
          collectionButton.type = "button";
          collectionButton.className = `preview-answer-chip preview-answer-chip--maq${isSelected ? " is-selected" : ""}`;
          collectionButton.disabled = requestInFlight || !blockIds.length;
          collectionButton.innerHTML = `
            <span class="preview-answer-chip-checkbox" aria-hidden="true">${isSelected ? "✓" : ""}</span>
            <span class="preview-answer-chip-text"></span>
          `;
          collectionButton.setAttribute("aria-pressed", isSelected ? "true" : "false");
          richText.appendInlineText(
            collectionButton.querySelector(".preview-answer-chip-text"),
            `${String(collection.title || "")} • ${Number(collection.block_count || blockIds.length)} block${Number(collection.block_count || blockIds.length) === 1 ? "" : "s"}`,
          );
          collectionButton.addEventListener("click", () => {
            applyTestModeCollection(collection);
          });
          collectionOptions.appendChild(collectionButton);
        });
        selectionPanel.appendChild(collectionOptions);
      }

      const blockLabel = document.createElement("p");
      blockLabel.className = "preview-written-answer-heading";
      blockLabel.textContent = `Course blocks • ${selectedCount} selected`;
      selectionPanel.appendChild(blockLabel);

      const blockOptions = document.createElement("div");
      blockOptions.className = "preview-message-options";
      blocks.forEach((block, index) => {
        const normalizedBlockId = String(block.id || "");
        const isSelected = testModeSelectedBlockIds.has(normalizedBlockId);
        const isLastSelected = isSelected && testModeSelectedBlockIds.size <= 1;
        const blockButton = document.createElement("button");
        blockButton.type = "button";
        blockButton.className = `preview-answer-chip preview-answer-chip--maq${isSelected ? " is-selected" : ""}`;
        blockButton.disabled = requestInFlight || isLastSelected;
        blockButton.innerHTML = `
          <span class="preview-answer-chip-checkbox" aria-hidden="true">${isSelected ? "✓" : ""}</span>
          <span class="preview-answer-chip-label">${optionLabel(index)}</span>
          <span class="preview-answer-chip-text"></span>
        `;
        blockButton.setAttribute("aria-pressed", isSelected ? "true" : "false");
        richText.appendInlineText(blockButton.querySelector(".preview-answer-chip-text"), block.title || "");
        blockButton.addEventListener("click", () => {
          toggleTestModeBlock(normalizedBlockId);
        });
        blockOptions.appendChild(blockButton);
      });
      selectionPanel.appendChild(blockOptions);

      const toggleAllRow = document.createElement("div");
      toggleAllRow.className = "preview-message-actions";
      const toggleAllButton = document.createElement("button");
      toggleAllButton.type = "button";
      toggleAllButton.className = "button secondary preview-confirm-button";
      toggleAllButton.textContent = "Toggle all blocks";
      toggleAllButton.disabled = requestInFlight;
      toggleAllButton.addEventListener("click", () => {
        toggleAllTestModeBlocks();
      });
      toggleAllRow.appendChild(toggleAllButton);
      selectionPanel.appendChild(toggleAllRow);

      article.appendChild(selectionPanel);
      return article;
    }

    if (message.kind === "test_setup_count") {
      appendFormattedMessageContent(article, message.text || "");
      const questionCount = testModeQuestionCount || defaultTestModeQuestionCountFromSession();
      const selectionPanel = document.createElement("div");
      selectionPanel.className = "preview-test-setup";

      const countLabel = document.createElement("p");
      countLabel.className = "preview-written-answer-heading";
      countLabel.textContent = `Question count • ${questionCount} selected`;
      selectionPanel.appendChild(countLabel);

      const countGrid = document.createElement("div");
      countGrid.className = "preview-test-count-grid";
      for (let count = 1; count <= maxTestModeQuestionCount(); count += 1) {
        const countButton = document.createElement("button");
        countButton.type = "button";
        countButton.className = `preview-test-count-chip${count === questionCount ? " is-selected" : ""}`;
        countButton.textContent = String(count);
        countButton.disabled = requestInFlight;
        countButton.setAttribute("aria-pressed", count === questionCount ? "true" : "false");
        countButton.addEventListener("click", () => {
          setTestModeQuestionCount(count);
        });
        countGrid.appendChild(countButton);
      }
      selectionPanel.appendChild(countGrid);
    ...
    if (message.kind === "confirm") {
      ...
      button.textContent = requestInFlight && activeAction === "confirm"
        ? "Starting..."
        : (message.button_label || "Confirm");
      button.disabled = requestInFlight || (isTestModeSetupActive() && testModeSelectedBlockIds.size <= 0);
      button.addEventListener("click", () => {
        const payload = isTestModeSetupActive()
          ? {
            selected_block_ids: Array.from(testModeSelectedBlockIds)
              .map((blockId) => Number.parseInt(String(blockId), 10))
              .filter((blockId) => Number.isFinite(blockId) && blockId > 0),
            question_count: testModeQuestionCount || defaultTestModeQuestionCountFromSession(),
          }
          : {};
        void postAction("confirm", payload);
      });
  ...
  function renderTranscript({ preserveScroll = false } = {}) {
    ...
    const previousScrollTop = preserveScroll ? transcriptNode.scrollTop : 0;
    ...
    syncQuestionViewport({ preserveScroll });
  }

  function renderSession() {
    normalizeCancelledSessionState();
    syncTestModeSetupStage();
    if (sessionState.completed) {
      setAwayTimestamp(0);
    }
    syncSelectedBlockIdsFromSession({
    ...
    updateProgress();
    if (validationMeta) {
      validationMeta.hidden = isTestModeSetupActive();
    }
    syncControls();
    syncOfflineValidationStatus();
  ...
  function cancelValidationSession() {
    if (sessionState.completed || !isValidationSessionMode() || cancellationInFlight) {
      return Promise.resolve();
    }
    if (isBrowserOffline()) {
      syncOfflineValidationStatus();
  ...
  function returnToPracticeMode() {
    ...
    if (
      String(sessionState.mode || "") === "validation_practice"
      && !sessionState.completed
      && !window.confirm("Leave Test Mode and return to practice? You will leave the test without feedback.")
    ) {
      return;
    }
    setStatus("");
    Promise.resolve(
      (!sessionState.completed && isValidationSessionMode())
  ...
  function handlePracticeFocusLoss() {
    if (!isPracticeValidationMode() || sessionState.completed) {
      return false;
    }
    practiceFocusReminderPending = true;
    showPracticeFocusReminder();
    return true;
  }
  ...
  nextButton?.addEventListener("click", () => {
    if (requestInFlight || (!isTestModeSetupActive() && !shouldShowNextButton())) {
      return;
    }
    if (isTestModeSetupActive()) {
      if (!isTestModeCountSetupStage()) {
        if (!testModeSelectedBlockIds.size) {
          setStatus("Choose at least one course block for Test Mode.");
          return;
        }
        testModeSetupStage = "count";
        renderSession();
        scrollTranscriptToBottom();
        return;
      }
  ...
  document.addEventListener("visibilitychange", () => {
    ...
    if (isPracticeValidationMode()) {
      if (document.hidden) {
        practiceFocusReminderPending = true;
      } else if (practiceFocusReminderPending) {
        showPracticeFocusReminder();
      }
      return;
    }
  ...
  window.addEventListener("focus", () => {
    if (!isPracticeValidationMode() || sessionState.completed || !practiceFocusReminderPending) {
      return;
    }
    showPracticeFocusReminder();
  });
  ...
  returnToPracticeButton?.addEventListener("click", () => {
  ...
  validationRoot.addEventListener("click", (event) => {
    ...
    if (!isPracticeValidationLaunchLink(link)) {
      ...
      if (!sessionState.completed && isValidationSessionMode() && href && !href.startsWith("#") && !href.toLowerCase().startsWith("javascript:")) {
        event.preventDefault();
        if (handlePracticeFocusLoss()) {
          return;
        }
  ...
  document.addEventListener("click", (event) => {
    const link = event.target instanceof Element ? event.target.closest("a") : null;
    if (!link || !(link instanceof HTMLAnchorElement) || validationRoot.contains(link)) {
      return;
    }
    if (event.defaultPrevented || link.target === "_blank" || link.hasAttribute("download") || event.metaKey || event.ctrlKey || event.shiftKey || event.altKey) {
      return;
    }
    const href = String(link.getAttribute("href") || "").trim();
    if (!sessionState.completed && isValidationSessionMode() && href && !href.startsWith("#") && !href.toLowerCase().startsWith("javascript:")) {
  ...
  if (!sidebarActiveBlockId && Array.isArray(sidebarPreviewState.blocks) && sidebarPreviewState.blocks.length) {
    sidebarActiveBlockId = String(sidebarPreviewState.active_block_id || sidebarPreviewState.blocks[0].id || "");
  }
  syncSelectedBlockIdsFromSession({ force: true });
  syncTestModeQuestionCountFromSession({ force: true });
  renderCourseMetrics();
  applySidebarState();
  renderSession();
