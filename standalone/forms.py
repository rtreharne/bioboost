...
class CourseConfigForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.fields.pop("question_bank_builder_enabled", None)
        self.fields.pop("question_bank_builder_auto_start", None)
        self.fields.pop("question_bank_builder_last_run_at", None)
        self.fields.pop("question_bank_builder_last_generated_at", None)
        self.fields.pop("question_bank_builder_last_error", None)
        self.fields.pop("question_bank_builder_daily_pair_cap", None)
        self.fields.pop("question_bank_builder_total_pair_cap", None)
        self.fields.pop("question_bank_builder_lease_expires_at", None)
...
class CourseBlockCollectionForm(forms.ModelForm):
    class Meta:
        model = CourseBlockCollection
        fields = ["title", "blocks"]

    def __init__(self, *args, course: Course, **kwargs):
        super().__init__(*args, **kwargs)
        self.course = course
        self.fields["title"].label = "Collection name"
        self.fields["title"].help_text = "Use a short label such as Module 1 or Paper 2 recap."
        self.fields["title"].widget.attrs.update(
            {
                "class": "course-setting-input",
                "placeholder": "E.g. Module 1",
            }
        )
        self.fields["blocks"].label = "Course blocks"
        self.fields["blocks"].help_text = "Choose the blocks this saved Test Mode collection should pre-select."
        self.fields["blocks"].queryset = course.blocks.order_by("order", "created_at", "pk")
        self.fields["blocks"].required = True
        self.fields["blocks"].label_from_instance = lambda block: block.title
        self.fields["blocks"].widget = forms.CheckboxSelectMultiple()
        self.fields["blocks"].widget.choices = self.fields["blocks"].choices

    def clean_title(self):
        title = str(self.cleaned_data.get("title") or "").strip()
        if not title:
            raise forms.ValidationError("Please enter a collection name.")
        existing = CourseBlockCollection.objects.filter(course=self.course, title__iexact=title)
        if self.instance.pk:
            existing = existing.exclude(pk=self.instance.pk)
        if existing.exists():
            raise forms.ValidationError("A collection with this name already exists for this course.")
        return title

    def clean_blocks(self):
        blocks = self.cleaned_data.get("blocks")
        if not blocks:
            raise forms.ValidationError("Choose at least one course block for this collection.")
        return blocks
