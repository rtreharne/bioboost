...
def course_block_avatar_upload_to(instance, filename: str) -> str:
    safe_name = Path(str(filename or "avatar.png")).name or "avatar.png"
    block_id = getattr(instance, "pk", None) or "unassigned"
    return f"standalone/block_avatars/{block_id}/{safe_name}"
...
class CourseBlock(TimeStampedModel):
    ...
    class AvatarGenerationStatus(models.TextChoices):
        IDLE = "idle", "Idle"
        QUEUED = "queued", "Queued"
        RUNNING = "running", "Running"
        COMPLETED = "completed", "Completed"
    ...
    avatar_file = models.FileField(upload_to=course_block_avatar_upload_to, blank=True)
    avatar_generation_status = models.CharField(
        max_length=20,
        choices=AvatarGenerationStatus.choices,
        default=AvatarGenerationStatus.IDLE,
    )
    avatar_generation_error = models.TextField(blank=True)
    avatar_generated_at = models.DateTimeField(null=True, blank=True)
    ...
    @property
    def avatar_fallback_label(self) -> str:
        parts = [part for part in str(self.title or "").split() if part]
        label = "".join(part[:1].upper() for part in parts[:2])
        return label or "B"
...
class CourseBlockCollection(TimeStampedModel):
    course = models.ForeignKey(Course, on_delete=models.CASCADE, related_name="block_collections")
    title = models.CharField(max_length=255)
    blocks = models.ManyToManyField(CourseBlock, related_name="collections", blank=True)

    class Meta:
        ordering = ["created_at", "pk"]
        constraints = [
            models.UniqueConstraint(fields=["course", "title"], name="unique_course_block_collection_title"),
        ]
