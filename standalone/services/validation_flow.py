...
from standalone.services.practice_validation import (
    practice_validation_enabled_question_types,
    question_is_practice_validation_eligible,
)
...
TEST_MODE_MAX_QUESTION_COUNT = 20
...
VALIDATION_STANDARD_WARNING_TEXT = (
    "You are about to start a validation. Navigating away from the validation at any time will cancel the validation and you will need to start again. "
    "You must not use any additional resources to answer the validation questions. You must not use any GAI tools to answer the validation questions. "
    "You are only permitted to use this device during the validation."
)
VALIDATION_NAVIGATION_CANCELLED_TEXT = (
...
def _selectable_block_payload(blocks: list[CourseBlock]) -> list[dict]:
    return [{"id": int(block.pk), "title": block.title} for block in blocks]


def _selectable_collection_payload(course, blocks: list[CourseBlock]) -> list[dict]:
    released_block_ids = {int(block.pk) for block in blocks}
    payload: list[dict] = []
    for collection in (
        CourseBlockCollection.objects.filter(course=course)
        .prefetch_related("blocks")
        .order_by("created_at", "pk")
    ):
        block_ids = [
            int(block_id)
            for block_id in collection.blocks.order_by("order", "created_at", "pk").values_list("pk", flat=True)
            if int(block_id) in released_block_ids
        ]
        if not block_ids:
            continue
        payload.append(
            {
                "id": int(collection.pk),
...
def _coerce_selected_block_ids(selected_block_ids, allowed_ids: set[int] | None = None) -> list[int]:
    if selected_block_ids is None:
        return []
    if isinstance(selected_block_ids, (str, bytes)):
        raw_ids = [selected_block_ids]
    else:
        try:
            raw_ids = list(selected_block_ids)
        except TypeError:
            raw_ids = [selected_block_ids]
    normalized: list[int] = []
...
def _normalize_selected_validation_blocks(
    released_blocks: list[CourseBlock],
    selected_block_ids,
    *,
    empty_error: str,
) -> list[CourseBlock]:
    if selected_block_ids is None:
        return list(released_blocks)
    allowed_ids = {int(block.pk) for block in released_blocks}
    normalized_ids = _coerce_selected_block_ids(selected_block_ids, allowed_ids)
    if not normalized_ids:
        raise ValidationFlowError(empty_error)
    return [block for block in released_blocks if int(block.pk) in normalized_ids]


def _normalize_test_mode_question_count(question_count, *, default_count: int) -> int:
    if question_count is None or question_count == "":
        normalized = int(default_count or 0)
    else:
        try:
            normalized = int(question_count)
        except (TypeError, ValueError):
            raise ValidationFlowError(f"Choose between 1 and {TEST_MODE_MAX_QUESTION_COUNT} questions for Test Mode.")
    if normalized < 1 or normalized > TEST_MODE_MAX_QUESTION_COUNT:
        raise ValidationFlowError(f"Choose between 1 and {TEST_MODE_MAX_QUESTION_COUNT} questions for Test Mode.")
    return normalized
...
def _seen_question_ids_for_enrollment(enrollment: Enrollment, course, *, ignore_question_ids=None) -> set[int]:
    seen_ids = {
        int(question_id)
    ...
    expanded_seen_ids = _expand_seen_pair_ids(seen_ids)
    ignored_ids = {
        int(question_id)
        for question_id in ignore_question_ids or []
        if question_id
    }
    if ignored_ids:
        expanded_seen_ids.difference_update(_expand_seen_pair_ids(ignored_ids))
    return expanded_seen_ids
...
def _scored_practice_validation_questions(
    course,
    enrollment: Enrollment,
    *,
    include_written: bool = True,
    blocks: list[CourseBlock] | None = None,
    ignore_question_ids=None,
):
    seen_ids = _seen_question_ids_for_enrollment(enrollment, course, ignore_question_ids=ignore_question_ids)
    questions = _filter_mismatched_coding_questions(list(_released_validation_questions(course, include_written=include_written, blocks=blocks)))
    scored = []
    for question in questions:
...
def _pick_practice_validation_questions(
    ...
    *,
    ...
    available = _scored_practice_validation_questions(
        course,
        enrollment,
        include_written=include_written,
        blocks=blocks,
        ignore_question_ids=ignore_question_ids,
    )
    released_blocks = list(blocks or _released_validation_blocks(course))
    selected = select_stratified_validation_questions(
        course,
...
def _practice_validation_round_type_targets(
    blocks: list[CourseBlock],
    quotas: dict[int, int],
    *,
    include_written: bool = True,
    ...
    for block in blocks:
        slot_count = max(0, int(quotas.get(block.pk) or 0))
        if slot_count <= 0:
            continue
        targets = _practice_validation_type_targets(block)
        enabled_types = set(practice_validation_enabled_question_types(block, include_written=include_written))
        for question_type in enabled_types:
            totals[question_type] += (float(targets.get(question_type, 0.0) or 0.0) * slot_count) / 100.0
    return totals


def _practice_validation_type_preference_for_slot(
    block: CourseBlock,
    selected_type_counts: dict[str, int],
    selected_type_counts_by_block: dict[int, dict[str, int]],
    round_type_targets: dict[str, float],
    *,
    slot_target: int,
    include_written: bool = True,
) -> list[str]:
    eligible_types = set(practice_validation_enabled_question_types(block, include_written=include_written))
    if not eligible_types:
        return []

    block_targets = _practice_validation_type_targets(block)
    block_type_counts = selected_type_counts_by_block[block.pk]
    candidates = []
    for question_type in (
        QuestionBankItem.QuestionType.MCQ,
        ...
        if question_type not in eligible_types:
            continue
        global_current_count = int(selected_type_counts.get(question_type, 0) or 0)
        global_remaining_quota = float(round_type_targets.get(question_type, 0.0) or 0.0) - global_current_count
        local_current_count = int(block_type_counts.get(question_type, 0) or 0)
        local_remaining_quota = (float(block_targets.get(question_type, 0.0) or 0.0) * max(1, slot_target) / 100.0) - local_current_count
        fallback_priority = {
            QuestionBankItem.QuestionType.MCQ: 0,
            QuestionBankItem.QuestionType.NUM: 1,
            QuestionBankItem.QuestionType.MAQ: 2,
        ...
        candidates.append((-global_remaining_quota, -local_remaining_quota, fallback_priority, question_type))
    candidates.sort()
    return [item[3] for item in candidates]


def _interleaved_block_slots(
    blocks: list[CourseBlock],
    quotas: dict[int, int],
    *,
    seed_key: str = "",
) -> list[CourseBlock]:
    slot_rows: list[tuple[int, str, int, int, CourseBlock]] = []
    for block in blocks:
        for slot_index in range(int(quotas.get(block.pk) or 0)):
            slot_rows.append(
                (
                    slot_index,
                    _stable_seed_token(seed_key, "practice-slot-order", block.pk),
                    block.order,
                    block.pk,
                    block,
                )
            )
    slot_rows.sort(key=lambda item: (item[0], item[1], item[2], item[3]))
    return [item[4] for item in slot_rows]


def _practice_validation_remaining_counts(
    questions_by_block_type: dict[int, dict[str, list[QuestionBankItem]]],
    blocks: list[CourseBlock],
    *,
    include_written: bool = True,
) -> dict[int, int]:
    ...
    for block in blocks:
        counts[block.pk] = sum(
            len(questions_by_block_type.get(block.pk, {}).get(question_type, []))
            for question_type in practice_validation_enabled_question_types(block, include_written=include_written)
        )
    return counts


def _pop_practice_validation_question(
    questions_by_block_type: dict[int, dict[str, list[QuestionBankItem]]],
    block: CourseBlock,
    question_types: list[str],
) -> QuestionBankItem | None:
    buckets = questions_by_block_type.get(block.pk, {})
    for question_type in question_types:
        bucket = buckets.get(question_type) or []
        if bucket:
            return bucket.pop(0)
    return None


def _pick_practice_validation_question_for_block(
    block: CourseBlock,
    questions_by_block_type: dict[int, dict[str, list[QuestionBankItem]]],
    selected_type_counts: dict[str, int],
    selected_type_counts_by_block: dict[int, dict[str, int]],
    ...
    *,
    ...
    preferred_types = _practice_validation_type_preference_for_slot(
        block,
        selected_type_counts,
        selected_type_counts_by_block,
        round_type_targets,
    ...
    if not preferred_types:
        preferred_types = [
            ...
            if question_type in practice_validation_enabled_question_types(block, include_written=include_written)
        ]
    question = _pop_practice_validation_question(
        questions_by_block_type,
        block,
        preferred_types[:1],
    )
    if question is None and len(preferred_types) > 1:
        question = _pop_practice_validation_question(
            questions_by_block_type,
            block,
            preferred_types[1:],
...
def select_stratified_practice_validation_questions(
    course,
    available_questions: list[QuestionBankItem],
    question_count: int,
    *,
    ...
    questions_by_block_type: dict[int, dict[str, list[QuestionBankItem]]] = defaultdict(lambda: defaultdict(list))
    for question in available_questions:
        if not question_is_practice_validation_eligible(question, include_written=include_written):
            continue
        questions_by_block_type[question.block_id][question.question_type].append(question)

    for block_id, buckets in questions_by_block_type.items():
        for question_type, bucket in buckets.items():
            bucket.sort(
                key=lambda question: (
                    _stable_seed_token(seed_key, "practice-pool", block_id, question_type, question.pk),
                    question.created_at,
                    question.pk,
                )
            )
    ...
    remaining_blocks = [
        ...
        if _practice_validation_remaining_counts(
            questions_by_block_type,
            [block],
            include_written=include_written,
        ).get(block.pk, 0) > 0
    ...
    remaining_counts = _practice_validation_remaining_counts(
        questions_by_block_type,
        remaining_blocks,
        include_written=include_written,
    )
    quotas = _allocate_weighted_block_slots(
        remaining_blocks,
        remaining_counts,
        question_count,
        seed_key=f"{seed_key}:practice-blocks:initial",
        ensure_one_min=False,
    )
    round_type_targets = _practice_validation_round_type_targets(
        remaining_blocks,
        quotas,
        include_written=include_written,
    )
    for block in _interleaved_block_slots(
        remaining_blocks,
        quotas,
        seed_key=f"{seed_key}:practice-blocks:initial",
    ):
        question = _pick_practice_validation_question_for_block(
            block,
            questions_by_block_type,
            selected_type_counts,
            selected_type_counts_by_block,
            round_type_targets,
            slot_target=max(1, int(quotas.get(block.pk) or 0)),
            include_written=include_written,
        )
        if question is None:
            continue
    ...
    while len(selected) < question_count:
        ...
        quotas = _allocate_weighted_block_slots(
            ...
            remaining_counts,
            missing,
            seed_key=f"{seed_key}:practice-blocks:backfill:{backfill_round}",
            ensure_one_min=False,
        )
        round_type_targets = _practice_validation_round_type_targets(
            remaining_blocks,
...
def _ensure_practice_validation_questions(
    ...
    *,
    ...
    block_list = list(blocks or _released_validation_blocks(course))
    if not block_list:
        return []
...
def _practice_instructions_confirmed(request, attempt: PracticeAttempt) -> bool:
    if request is None:
        return False
    if attempt.completed_at:
        return True
    return bool(_ui_attempt_state(request, _practice_ui_key(attempt)).get("instructions_confirmed"))


def _practice_invalidated_reason(request, attempt: PracticeAttempt) -> str:
    if request is None:
        return ""
    return str(_ui_attempt_state(request, _practice_ui_key(attempt)).get("invalidated_reason") or "").strip()
...
def _validation_practice_question_count(course) -> int:
    ...
    return max(1, min(TEST_MODE_MAX_QUESTION_COUNT, question_count))


def _practice_attempt_question_count(attempt: PracticeAttempt) -> int:
    existing_count = int(attempt.attempt_questions.count() or 0)
    if existing_count > 0:
        return max(1, min(TEST_MODE_MAX_QUESTION_COUNT, existing_count))
    return _validation_practice_question_count(attempt.enrollment.course)


def _practice_selected_question_count(attempt: PracticeAttempt, *, request=None) -> int:
    default_count = _practice_attempt_question_count(attempt)
    if request is not None:
        stored_count = _ui_attempt_state(request, _practice_ui_key(attempt)).get("selected_question_count")
        try:
            return _normalize_test_mode_question_count(stored_count, default_count=default_count)
        except ValidationFlowError:
            pass
    return default_count


def _practice_selected_blocks(
    attempt: PracticeAttempt,
    *,
    request=None,
    released_blocks: list[CourseBlock] | None = None,
) -> list[CourseBlock]:
    block_list = list(released_blocks or _released_validation_blocks(attempt.enrollment.course))
    if not block_list:
        return []
    allowed_ids = {int(block.pk) for block in block_list}
    instructions_confirmed = _practice_instructions_confirmed(request, attempt) if request is not None else bool(attempt.completed_at)
    if request is not None:
        stored_ids = _coerce_selected_block_ids(
            _ui_attempt_state(request, _practice_ui_key(attempt)).get("selected_block_ids"),
            allowed_ids,
        )
        if stored_ids:
            return [block for block in block_list if int(block.pk) in stored_ids]
    if not instructions_confirmed and not attempt.completed_at:
        return block_list
    question_block_ids = _coerce_selected_block_ids(
        attempt.attempt_questions.order_by("order", "created_at").values_list("question__block_id", flat=True),
        allowed_ids,
    )
    if question_block_ids:
        return [block for block in block_list if int(block.pk) in question_block_ids]
    return block_list


def _rebuild_practice_validation_attempt_questions(
    attempt: PracticeAttempt,
    *,
    blocks: list[CourseBlock],
    question_count: int,
) -> None:
    current_question_ids = list(attempt.attempt_questions.values_list("question_id", flat=True))
    locked_questions = _ensure_practice_validation_questions(
        attempt.enrollment.course,
        attempt.enrollment,
        question_count,
        include_written=True,
        seed_key=_practice_attempt_seed(attempt),
        blocks=blocks,
        ignore_question_ids=current_question_ids,
    )
    if len(locked_questions) < question_count:
        raise ValidationFlowError(
            "Not enough released validation questions are available for the selected course blocks yet. Choose more blocks for Test Mode."
        )
    attempt.attempt_questions.all().delete()
    for order, question in enumerate(locked_questions, start=1):
...
def get_or_create_validation_practice_attempt(enrollment: Enrollment) -> PracticeAttempt:
    ...
    question_count = _validation_practice_question_count(enrollment.course)
...
def _practice_validation_review_transcript(attempt: PracticeAttempt, *, request=None) -> list[dict]:
    ...
    impact_text = (
        "**Projected overall course score** from this **Test Me** session: "
    ...
    if applied_floor:
        impact_text += (
            " Because your practice score is below **40.0%** but this Test Me session is **40.0% or above**, "
            "your projected overall score is lifted to **40.0%**."
        )
    else:
        impact_text += f" Your projected overall score is **{projected_score:.1f}%**."
    impact_text += " This is based on this **Test Me** session only. You still need to complete a **real validation**."
    transcript = [
        {
            ...
            "kind": "summary",
            "text": (
                f"**Test Me complete.** You scored **{validation_score:.1f}%** "
...
def _official_instruction_transcript() -> list[dict]:
    messages = [
        {
            "id": "validation-standard-warning",
            "role": "assistant",
            "kind": "text",
            "text": VALIDATION_STANDARD_WARNING_TEXT,
        }
    ]
...
def serialize_validation_practice_session(attempt: PracticeAttempt, *, request=None) -> dict:
    ...
    selected_blocks = _practice_selected_blocks(attempt, request=request, released_blocks=released_blocks)
    selectable_blocks = _selectable_block_payload(released_blocks)
    selectable_collections = _selectable_collection_payload(attempt.enrollment.course, released_blocks)
    selected_block_ids = [int(block.pk) for block in selected_blocks]
    selected_block_titles = [block.title for block in selected_blocks]
    selected_question_count = _practice_selected_question_count(attempt, request=request)
    invalidated_reason = _practice_invalidated_reason(request, attempt)
    if invalidated_reason:
        return {
            ...
            "transcript": [
                {
                    "id": f"practice-validation-cancelled-{attempt.pk}",
                    "role": "assistant",
                    "kind": "summary",
            ...
            "waq_draft": {},
            "selectable_blocks": selectable_blocks,
            "selectable_collections": selectable_collections,
            "selected_block_ids": selected_block_ids,
            "selected_blocks": selected_block_titles,
            "selected_question_count": selected_question_count,
            "max_question_count": TEST_MODE_MAX_QUESTION_COUNT,
    ...
    instructions_confirmed = _practice_instructions_confirmed(request, attempt)
    transcript = []
    if not instructions_confirmed:
        transcript.extend(
            [
                {
                    "id": f"practice-validation-setup-{attempt.pk}",
                    "role": "assistant",
                    "kind": "test_setup",
                    "text": "Choose the course blocks to include, then choose how many questions to generate for this Test Mode run.",
                    "button_label": "Generate Test Mode",
                },
            ]
        )
    ...
    if instructions_confirmed and current and not hide_next_question:
        visible_question = current
    elif instructions_confirmed and hide_next_question and latest_answered:
    ...
    return {
        ...
        "invalidated_reason": "",
        "next_available": bool(instructions_confirmed and hide_next_question and (current or latest_answered)),
    }
...
def draft_validation_practice_answer(request, attempt: PracticeAttempt, question_id: int, answer_text: str) -> dict:
    if not _practice_instructions_confirmed(request, attempt):
...
def confirm_validation_practice_instructions(
    request,
    attempt: PracticeAttempt,
    *,
    ...
    if attempt.completed_at or _practice_instructions_confirmed(request, attempt):
        return serialize_validation_practice_session(attempt, request=request)
    released_blocks = _released_validation_blocks(attempt.enrollment.course)
    selected_blocks = _normalize_selected_validation_blocks(
        released_blocks,
        selected_block_ids,
        empty_error="Choose at least one course block for Test Mode.",
    )
    normalized_question_count = _normalize_test_mode_question_count(
        question_count,
        default_count=_practice_attempt_question_count(attempt),
    )
    _rebuild_practice_validation_attempt_questions(
        attempt,
        blocks=selected_blocks,
        question_count=normalized_question_count,
    )
    _set_ui_attempt_state(
        request,
        _practice_ui_key(attempt),
        instructions_confirmed=True,
        invalidated_reason="",
        awaiting_next=False,
        question_messages={},
        selected_block_ids=[int(block.pk) for block in selected_blocks],
        selected_question_count=normalized_question_count,
    )
    return serialize_validation_practice_session(attempt, request=request)


def cancel_validation_practice_attempt(request, attempt: PracticeAttempt) -> dict:
    current_question = _practice_current_question(attempt)
    if current_question is not None:
        _clear_draft(request, "practice", attempt.pk, current_question.question_id)
    _clear_practice_next_step(request, attempt)
    _set_ui_attempt_state(
        request,
        _practice_ui_key(attempt),
        instructions_confirmed=True,
        invalidated_reason=VALIDATION_NAVIGATION_CANCELLED_TEXT,
    )
    return serialize_validation_practice_session(attempt, request=request)
...
@transaction.atomic
def cancel_official_validation_attempt(request, attempt: ValidationAttempt) -> dict:
    attempt = ValidationAttempt.objects.select_related("event", "enrollment", "enrollment__course").get(pk=attempt.pk)
    if attempt.status != ValidationAttempt.Status.IN_PROGRESS:
        return serialize_official_validation_session(attempt, request=request)
    _void_attempt(attempt, VALIDATION_NAVIGATION_CANCELLED_TEXT)
    return serialize_official_validation_session(attempt, request=request)
