...
def _normalize_numeric_explanation_prose(text: str) -> str:
    cleaned = normalize_physics_numeric_prose(_decode_literal_unicode_escapes(text))
    if not cleaned:
        return ""
...
NUMERIC_RECENT_ANGLE_ERROR = "OpenAI generated a repeated numerical MCQ angle too similar to recent questions in this block."
...
def _is_repeated_numeric_angle_error(error_message: str) -> bool:
    return "repeated numerical mcq angle too similar to recent questions in this block" in str(error_message or "").lower()


def _numeric_similarity_fingerprint_from_payload(payload: dict) -> str:
    numeric_metadata = payload.get("numeric_metadata") if isinstance(payload.get("numeric_metadata"), dict) else {}
    correct_answers = payload.get("correct_answers") or []
    fingerprint_payload = {
        "stem": re.sub(r"\s+", " ", str(payload.get("stem", "")).strip()).lower(),
        "correct_answer": str(correct_answers[0]).strip().lower() if correct_answers else "",
        "formula_focus": _numeric_formula_focus_from_metadata(numeric_metadata),
    }
...
def _accept_numeric_similarity_fallback(
    *,
    course: Course,
    block: CourseBlock,
    chunk: ContentChunk,
    ...
    try:
        ...
        raise
    except (NumericQuestionValidationError, ValueError) as exc:
        return None, None, str(exc)

    if practice is None or validation is None:
        return None, None, "The generated numerical MCQ duplicated an existing question."

    _trace_generation(
        "numeric_similarity_fallback_accepted",
        block=block.title,
        block_id=block.pk,
        objective=(objective.text if objective else ""),
        chunk_id=chunk.pk,
        ...
        correct_answer=(payload.get("correct_answers") or [""])[0],
        formula_focus=_numeric_formula_focus_from_metadata(payload.get("numeric_metadata")),
    )
...
def _attempt_numeric_generation_for_chunk(
    *,
    ...
    existing_hashes: set[str],
    avoid_question_angles: list[str] | None = None,
    allow_numeric_recent_angle_fallback: bool = False,
) -> tuple[QuestionBankItem | None, QuestionBankItem | None, str]:
    numeric_avoid_question_angles = list(avoid_question_angles or [])
    last_generation_error = ""
    last_repeated_angle_payload: dict | None = None
    last_repeated_angle_fingerprint = ""
    repeated_angle_attempt_count = 0
    saw_repeated_angle_failure = False
    saw_non_repeated_angle_failure = False
    ...
    for _attempt_index in range(NUMERIC_GENERATION_ATTEMPT_LIMIT):
        ...
        try:
            ...
            saw_non_repeated_angle_failure = True
            _trace_generation(
            ...
            if _is_repeated_numeric_angle_error(last_generation_error):
                saw_repeated_angle_failure = True
                repeated_angle_attempt_count += 1
                if payload is not None:
                    last_repeated_angle_payload = payload
                    current_fingerprint = _numeric_similarity_fingerprint_from_payload(payload)
                    if (
                        allow_numeric_recent_angle_fallback
                        and current_fingerprint == last_repeated_angle_fingerprint
                    ):
                        practice, validation, fallback_error = _accept_numeric_similarity_fallback(
                            course=course,
                            block=block,
                            chunk=chunk,
                            objective=objective,
                            payload=payload,
                            existing_hashes=existing_hashes,
                            attempts=_attempt_index + 1,
                            fallback_reason="matching_retry",
                        )
                        if practice is not None and validation is not None:
                            return practice, validation, ""
                        last_generation_error = fallback_error
                        saw_non_repeated_angle_failure = True
                        last_repeated_angle_payload = None
                        break
                    last_repeated_angle_fingerprint = current_fingerprint
            else:
                saw_non_repeated_angle_failure = True
    ...
    if (
        allow_numeric_recent_angle_fallback
        and saw_repeated_angle_failure
        and not saw_non_repeated_angle_failure
        and last_repeated_angle_payload is not None
    ):
        practice, validation, fallback_error = _accept_numeric_similarity_fallback(
            course=course,
            block=block,
            chunk=chunk,
            objective=objective,
            payload=last_repeated_angle_payload,
            existing_hashes=existing_hashes,
            attempts=repeated_angle_attempt_count or NUMERIC_GENERATION_ATTEMPT_LIMIT,
            fallback_reason="attempt_limit",
        )
        if practice is not None and validation is not None:
            return practice, validation, ""
        last_generation_error = fallback_error
        ...
        for chunk in ranked_chunks:
            ...
            if question_type == QuestionBankItem.QuestionType.NUM:
                practice, validation, numeric_error = _attempt_numeric_generation_for_chunk(
                    ...
                    existing_hashes=existing_hashes,
                    avoid_question_angles=avoid_question_angles,
                    allow_numeric_recent_angle_fallback=allow_numeric_recent_angle_fallback,
                )
                if practice is not None and validation is not None:
                    return practice, validation, ""
                if numeric_error:
...
def _ensure_standard_coding_fallback_types(ordered_question_types: list[str]) -> list[str]:
    ordered = list(ordered_question_types)
...
def _create_question_pair(
    *,
    ...
    if (
        ...
        raise NumericQuestionValidationError(NUMERIC_RECENT_ANGLE_ERROR)
