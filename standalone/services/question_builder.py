...
class PracticeValidationReadiness:
    ...
    def detail(self) -> str:
        if self.ready:
            return (
                f"Test Me is ready. "
                f"{self.ready_count:,} approved released validation questions are available."
            )
        return (
            f"Test Me unlocks at {self.threshold:,} approved released validation questions. "
...
def practice_validation_readiness(course: Course, *, today=None) -> PracticeValidationReadiness:
    ...
    ready_count = released_validation_pool_count(course, approved_only=True, today=today)
...
def run_course_question_bank_builder_pass(course_id: int, *, now=None) -> QuestionBankBuilderPassResult:
    now = now or timezone.now()
    if course_import_work_is_active(course_id=course_id):
