...
_TASK_LABELS = {
    ...
    "block_avatar_generation": "Block avatar generation",
    "course_block_avatar_generation": "Course avatar generation",
...
def _run_registered_task(task_name: str, *args) -> None:
    from standalone.tasks import (
        run_block_avatar_generation,
        run_block_creation_processing,
        run_course_block_avatar_generation,
    ...
    if task_name == "block_avatar_generation":
        ...
        return
    if task_name == "course_block_avatar_generation":
        run_course_block_avatar_generation(int(args[0]), force=bool(args[1]) if len(args) > 1 else False)
        return
