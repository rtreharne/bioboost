...
def _preview_selected_blocks(course: Course, state: dict, *, released_blocks: list[CourseBlock] | None = None) -> list[CourseBlock]:
    block_list = list(released_blocks or _released_blocks(course))
    if not block_list:
        return []
    allowed_ids = {int(block.pk) for block in block_list}
    stored_ids = _coerce_selected_block_ids(state.get("selected_block_ids"), allowed_ids)
    if stored_ids:
        return [block for block in block_list if int(block.pk) in stored_ids]
    question_ids = [int(question_id) for question_id in state.get("question_ids") or [] if question_id]
    if question_ids:
        question_block_ids = _coerce_selected_block_ids(
            _question_queryset(course).filter(pk__in=question_ids).values_list("block_id", flat=True),
            allowed_ids,
        )
...
def _preview_validation_default_question_count() -> int:
    return max(1, min(TEST_MODE_MAX_QUESTION_COUNT, int(getattr(settings, "PREVIEW_VALIDATION_DEFAULT_QUESTION_COUNT", 10) or 10)))


def _preview_selected_question_count(state: dict) -> int:
    default_count = max(
        1,
        min(
            TEST_MODE_MAX_QUESTION_COUNT,
            int(state.get("selected_question_count") or len(state.get("question_ids") or []) or _preview_validation_default_question_count()),
        ),
    )
    try:
        return _normalize_test_mode_question_count(state.get("selected_question_count"), default_count=default_count)
    except ValidationFlowError:
        return default_count


def _pick_preview_validation_practice_questions(
    request,
    course: Course,
    question_count: int,
    *,
    ...
    released_blocks = list(blocks or _released_blocks(course))
    if not released_blocks:
        return []
    seen_ids = _preview_seen_question_ids(request, course)
    seen_ids.difference_update(int(question_id) for question_id in ignore_question_ids or [] if question_id)
    available = [
        question
        for question in _question_queryset(course)
...
def _initial_preview_state(request, course: Course) -> dict:
    ...
    question_ids = _pick_preview_validation_practice_questions(request, course, question_count, blocks=released_blocks)
    ...
    return {
        ...
        "selected_block_ids": [int(block.pk) for block in released_blocks],
    ...
    transcript = [
        {
            ...
            "role": "assistant",
            "kind": "summary",
            "text": f"**Test Me complete.** You scored **{validation_score:.1f}%** (**{correct_count} of {len(question_ids)}**).",
        },
        {
...
def _completed_preview_validation_session(request, course: Course, state: dict) -> dict:
    ...
    released_blocks = _released_blocks(course)
    selected_blocks = _preview_selected_blocks(course, state, released_blocks=released_blocks)
    selected_question_count = _preview_selected_question_count(state)
    return {
        ...
        "selectable_collections": _selectable_collection_payload(course, released_blocks),
        "selected_block_ids": [int(block.pk) for block in selected_blocks],
        "selected_blocks": [block.title for block in selected_blocks],
...
def preview_validation_history_session(request, course: Course, history_id: int) -> dict:
    ...
    raise ValidationFlowError("That Test Me review is no longer available.")
...
def serialize_preview_validation_state(request, course: Course) -> dict:
    ...
    if invalidated_reason:
        return {
            ...
            "transcript": [
                {
                    "id": f"preview-validation-cancelled-{course.pk}",
                    "role": "assistant",
                    "kind": "summary",
    ...
    if not instructions_confirmed:
        transcript.extend(
            [
                {
                    "id": f"preview-validation-setup-{course.pk}",
                    "role": "assistant",
                    "kind": "test_setup",
    ...
    complete = False
    score = 0.0
    pending_question = None if complete or not instructions_confirmed else _pending_question(course, state)
    latest_answered = _latest_answered_question(course, state)
    visible_question = pending_question
    if instructions_confirmed and next_available and latest_answered:
    ...
    return {
        ...
        "invalidated_reason": "",
        "next_available": bool(instructions_confirmed and next_available and (pending_question or latest_answered) and not complete),
    }
...
def confirm_preview_validation_practice(request, course: Course, *, selected_block_ids=None, question_count=None) -> dict:
    state = _course_state(request, course)
    if state.get("completed") or state.get("instructions_confirmed"):
        return serialize_preview_validation_state(request, course)
    released_blocks = _released_blocks(course)
    ...
    normalized_question_count = _normalize_test_mode_question_count(
        question_count,
        default_count=_preview_selected_question_count(state),
    )
    question_ids = _pick_preview_validation_practice_questions(
        request,
        course,
        normalized_question_count,
        blocks=selected_blocks,
        ignore_question_ids=state.get("question_ids") or [],
    )
    if len(question_ids) < normalized_question_count:
    ...
    state["selected_block_ids"] = [int(block.pk) for block in selected_blocks]
    state["selected_question_count"] = normalized_question_count
...
def cancel_preview_validation_practice(request, course: Course) -> dict:
    state = _course_state(request, course)
    state["invalidated_reason"] = VALIDATION_NAVIGATION_CANCELLED_TEXT
    state["instructions_confirmed"] = True
    state["next_available"] = False
    request.session.modified = True
    return serialize_preview_validation_state(request, course)
...
def serialize_preview_student_validate_state(request, course: Course, event) -> dict:
    ...
    if invalidated_reason:
        return {
            ...
            "transcript": [
                {
                    "id": f"preview-student-validate-cancelled-{course.pk}",
                    "role": "assistant",
                    "kind": "summary",
                    "text": invalidated_reason,
                }
    ...
    if not attendance_confirmed:
        transcript.append(
            {
                "id": f"preview-student-validate-standard-warning-{course.pk}",
                "role": "assistant",
                "kind": "text",
...
def cancel_preview_student_validate(request, course: Course, event) -> dict:
    state = _course_student_validate_state(request, course, event)
    state["invalidated_reason"] = VALIDATION_NAVIGATION_CANCELLED_TEXT
    state["instructions_confirmed"] = True
    state["attendance_confirmed"] = False
    state["next_available"] = False
    request.session.modified = True
    return serialize_preview_student_validate_state(request, course, event)
